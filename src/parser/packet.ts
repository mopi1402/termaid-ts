...
export function parsePacket(text: string): Packet {
  ...
  for (let line of lines.slice(1)) {
    ...
    if (range !== null) {
      ...
      packet.fields.push({ start: Number.parseInt(range[1] as string, 10), end, label: (range[3] as string).trim() });
      next = end + 1;
      continue;
    }
    ...
    if (relative !== null) {
      ...
      packet.fields.push({ start: next, end, label: (relative[2] as string).trim() });
      next = end + 1;
      continue;
    }
    ...
    if (single !== null) {
      ...
      packet.fields.push({ start, end: start, label: (single[2] as string).trim() });
      next = start + 1;
    }
  }
