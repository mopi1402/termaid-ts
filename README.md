...
same source, the same width and the same theme produce the same bytes as the reference itself, colour codes included.
...
is a dependency of the package, so installing it costs a consumer nothing.
